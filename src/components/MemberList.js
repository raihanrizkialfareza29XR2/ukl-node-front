...
const MemberList = () => {
    ...
    .filter(member => {
        ...
        else if (member.transaksi.kode_invoice.toLowerCase().includes(search.toLowerCase())){
            return member
        }
    })
    .map((member, index) => {
        return (
            <tr key={member.id_member}>
                ...
                <td>
                    <Link to={`/member/edit/${member.id_member}`} className="btn btn-primary">Edit</Link>
                    |
                    <button onClick={ () => deleteMember(member.id_member) } className="btn btn-danger btn-sm">Delete</button>
                </td>
            </tr>
        )
    })
  ...
  return (
    <div className="table table-responsive">
        <Link to="/outlet/add" className="btn btn-primary mb-5">Add</Link>
        <input 
        type="text" 
